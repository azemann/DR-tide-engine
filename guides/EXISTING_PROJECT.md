...
L’intégration commence seulement après validation humaine de ce rapport.

## 3. Installer la méthode sans collision
...
- différer l’intégration avec une raison.
...
## 6. Converger vers le noyau commun

Le projet rejoint le parcours commun au niveau `development` lorsque :
...
À partir de là, l’origine `existing` n’ajoute plus d’exigence particulière. Release, distribution et exploitation restent activées séparément selon la réalité.
